...
fun UpdateListDialog(
    ...
    LaunchedEffect(true) {
        viewModel.setSelectedColor(category.backgroundColor!!)
        viewModel.setTfState(category.categoryName!!)
    }
    Surface(
        modifier = modifier
            .fillMaxWidth(0.75f),
        ...
        Column(
            ...
            OutlinedTextField(
                modifier = Modifier
                    .fillMaxWidth(),
                value = viewModel.updateTFState,
                onValueChange = {
                    viewModel.setTfState(it)
                },
                label = {
            ...
            LazyRow(
                ...
                itemsIndexed(colors) { index, color ->
                    UpdateColorPickerItem(color = color, viewModel = viewModel)
                }
            }
...
@Composable
fun UpdateColorPickerItem(color: Int, viewModel: ViewListViewModel) {
    Surface(
        modifier = Modifier
            .width(30.dp)
            .height(30.dp)
            .clickable {
                viewModel.setSelectedColor(color = color)
            },
